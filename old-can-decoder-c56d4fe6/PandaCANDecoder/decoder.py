...
from PandaCANDecoder.plotter import (
    plot_bit_table,
    plot_probability_chart,
    plot_ts_signal,
    plot_ts_signal_comparison,
    plot_ts_signal_multi,
)
from PandaCANDecoder.signal import Signal
from PandaCANDecoder.utils import (
    convert_signal,
    validate_byte_order,
...
class Decoder:
    ...
    def __init__(self, csv_file_path=None):
        ...
        self.time_series_msg_dir = "./old_time_series_msgs"  # TODO: make static
        self.msgs = []
    ...
    def generate_msgs(self):
        ...
        """
        unique_pairings = self.all_data.drop_duplicates(
            subset=["MessageID", "PandaNum", "Bus", "MessageLength"]
        )
        ...
        for msg_id in tqdm(unique_msg_ids, desc="Generating messages".ljust(30)):
            ...
            self.msgs.append(
                Message(msg_id=msg_id, panda_buses=panda_buses, msg_length=msg_length)
            )
    ...
    def print_msgs(self):
        ...
        if not self.msgs:
            print(
                "WARNING: No messages have been generated. Use Decoder.generate_msgs() to do so."
            )
            return
    ...
    def _calculate_signedness(self, method, signal, gamma):
        ...
        if method == "msb_classifier":
            ...
            ts_bin = (
                (
                    (
                        ts_signal_data_raw[:, None]
                        & (1 << np.arange(signal.length, dtype="uint64")[::-1])
                    )
                )
                > 0
            ).astype(int)
            ...
            # conditions
            prob_1_0 = (
                np.count_nonzero((ts_bin[:, 0] == 1) & (ts_bin[:, 1] == 0))
                / msg.msg_quantity
            )
            prob_0_1 = (
                np.count_nonzero((ts_bin[:, 0] == 0) & (ts_bin[:, 1] == 1))
                / msg.msg_quantity
            )
            ...
            prob_0_0_and_1_1 = (
                np.count_nonzero(bool_0_0[:-1] & bool_1_1[1:]) / msg.msg_quantity
            )
        ...
        for msg in self.msgs:
            for signal_a in msg.signals:
                if signal_a == signal_b:
                    continue
                model, r_sq = self.signal_match(
                    signal_a, signal_b, start_time, end_time
                )
                if r_sq > thresh:
                    if print_sig:
    ...
    def signal_match(self, signal_a, signal_b, start_time, end_time):
        ...
        """
        x_time_idx = np.where(
            (signal_a.ts_data_timestamps > start_time)
            & (signal_a.ts_data_timestamps < end_time)
        )
        y_time_idx = np.where(
            (signal_b.ts_data_timestamps > start_time)
            & (signal_b.ts_data_timestamps < end_time)
        )
